...
export default function CandidateDetailsPage() {
  ...
  const fetchApplications = async () => {
    ...
    const res = await fetch(
      `${API_BASE}/applications?candidate_id=${params.id}`,
      { headers: { Authorization: `Bearer ${token}` } }
    );
  ...
  const {
    ...
    // Poll every 3 seconds if parsed_data is absent (resume still processing)
  ...
  const { data: applications = [], isLoading: loadingApps } = useQuery({
    ...
    refetchInterval: (query) => {
      if (
        query.state.data &&
        query.state.data.some((a: any) => a.score === null)
      )
        return 3000;
      return false;
    },
    ...
    return (
      <div className="p-8 max-w-7xl mx-auto text-center">
        ...
        <p className="text-zinc-500 mt-2">
          {errorCandidate ? "Could not load candidate details." : "Candidate not found"}
        </p>
  ...
  const aiSummary =
    parsedData.summary ||
    "No AI summary available. Resume might still be processing.";
  const skills: string[] = parsedData.skills || [];
  const experience: any[] = parsedData.experience || [];
  const education: any[] = parsedData.education || [];

  // Use the highest score among their applications as their overall match
  const bestMatch =
    applications.length > 0
      ? Math.max(...applications.map((a: any) => (a.score || 0) * 100))
      : 0;
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 shrink-0">
        <div>
          <Link
            ...
            className="flex items-center gap-2 text-sm text-zinc-500 hover:text-zinc-900 mb-2 transition-colors"
          >
            ...
            <div className="flex items-center gap-2 mt-1">
              <div className="w-24 h-2.5 bg-zinc-200 rounded-full overflow-hidden">
                <div
                  ...
                  style={{ width: `${Math.min(100, bestMatch)}%` }}
                />
              </div>
          ...
          <Tabs defaultValue="insights" className="flex-1 flex flex-col h-full">
            <CardHeader className="shrink-0 py-0 px-6 border-b border-zinc-100 bg-zinc-50/50">
              <TabsList className="bg-transparent border-b-0 h-14 space-x-6 pb-0 px-0 w-full justify-start">
                <TabsTrigger
                  value="insights"
                  className="data-[state=active]:border-b-2 data-[state=active]:border-zinc-900 rounded-none h-full data-[state=active]:shadow-none data-[state=active]:bg-transparent"
                >
                  AI Insights
                </TabsTrigger>
                <TabsTrigger
                  ...
                  {experience.length > 0 ? (
                    <div className="space-y-6">
                      {experience.map((exp, i) => (
                        <div
                          key={i}
                          className="relative pl-4 border-l-2 border-zinc-200 pb-2"
                        >
                          <div className="absolute w-2.5 h-2.5 bg-zinc-200 rounded-full -left-[6px] top-1.5 ring-4 ring-white" />
