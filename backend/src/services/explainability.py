...
class ExplainabilityService:
    ...
    async def generate_explanation(
        ...
        recommendations = self._generate_recommendations(candidate, job, score_data, strengths, weaknesses)
    ...
    def _identify_weaknesses(
        ...
        if features.get("experience_match", 0) < 0.5:
            if self._infer_candidate_experience(candidate) < self._infer_job_experience(job):
        ...
        if not candidate.parsed_data:
            weaknesses.append("Resume could not be fully parsed; information may be incomplete")
    ...
    def _generate_recommendations(
        ...
        if match_score > 80:
            ...
            recommendations.append("Consider this candidate for team lead consideration if applicable.")

        elif match_score > 60:
            recommendations.append("Good candidate fit. Proceed with initial screening.")
            recommendations.append("Consider discussing specific skill gaps in first interview.")

        elif match_score > 40:
            recommendations.append("Fair match. Phone screen to assess potential and learning ability.")
            recommendations.append("Verify claimed skills and recent project experience.")

        else:
            recommendations.append("Below threshold match. Consider for future roles.")
            recommendations.append("Save for future matching when experience aligns better.")

        # Add specific recommendations based on weaknesses
        if any("skills" in w.lower() for w in weaknesses):
            recommendations.append("Ask about willingness to quickly learn missing skills.")

        if any("relocation" in w.lower() for w in weaknesses):
            recommendations.append("Clarify remote work options or relocation assistance.")
    ...
    def _generate_summary(
        ...
        confidence_str = (
            "High confidence"
            if confidence > 0.7
            else "Moderate confidence" if confidence > 0.4 else "Low confidence"
        )
