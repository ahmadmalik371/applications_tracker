...
down_revision: str | Sequence[str] | None = "8d3e9f0a4b2c"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "max_resume_processing", sa.Integer(), nullable=False, server_default="50"
        ),
        sa.Column(
            "max_ai_requests", sa.Integer(), nullable=False, server_default="100"
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "usage_resume_processing", sa.Integer(), nullable=False, server_default="0"
        ),
        sa.Column(
            "usage_ai_requests", sa.Integer(), nullable=False, server_default="0"
        ),
    ...
    op.create_index(
        op.f("ix_org_feature_flag_overrides_organization_id"),
        "org_feature_flag_overrides",
        ["organization_id"],
    )
    op.create_index(
        op.f("ix_org_feature_flag_overrides_feature_flag_id"),
        "org_feature_flag_overrides",
        ["feature_flag_id"],
    )
    ...
    op.create_index(
        op.f("ix_activity_timeline_entity_type"), "activity_timeline", ["entity_type"]
    )
    op.create_index(
        op.f("ix_activity_timeline_entity_id"), "activity_timeline", ["entity_id"]
    )
    op.create_index(
        op.f("ix_activity_timeline_organization_id"),
        "activity_timeline",
        ["organization_id"],
    )
    op.create_index(
        op.f("ix_activity_timeline_actor_id"), "activity_timeline", ["actor_id"]
    )
    ...
    op.create_index(
        op.f("ix_ai_recommendation_feedback_organization_id"),
        "ai_recommendation_feedback",
        ["organization_id"],
    )
    op.create_index(
        op.f("ix_ai_recommendation_feedback_recruiter_id"),
        "ai_recommendation_feedback",
        ["recruiter_id"],
    )
    ...
    op.create_index(
        op.f("ix_bias_reports_organization_id"), "bias_reports", ["organization_id"]
    )
    op.create_index(
        op.f("ix_bias_reports_model_version_id"), "bias_reports", ["model_version_id"]
    )
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_candidates_embedding ON candidates USING ivfflat (embedded vector_cosine_ops) WITH (lists = 100);"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_jobs_embedding ON jobs USING ivfflat (embedding vector_cosine_ops) WITH (lists = 100);"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_candidates_tsv ON candidates USING gin (to_tsvector('english', coalesce(first_name,'') || ' ' || coalesce(last_name,'') || ' ' || email));"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_jobs_tsv ON jobs USING gin (to_tsvector('english', coalesce(title,'') || ' ' || coalesce(description,'')));"
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        op.f("ix_model_evaluations_model_version_id"), table_name="model_evaluations"
    )
    ...
    op.drop_index(
        op.f("ix_ai_recommendation_feedback_recruiter_id"),
        table_name="ai_recommendation_feedback",
    )
    op.drop_index(
        op.f("ix_ai_recommendation_feedback_organization_id"),
        table_name="ai_recommendation_feedback",
    ...
    op.drop_index(
        op.f("ix_activity_timeline_organization_id"), table_name="activity_timeline"
    )
    op.drop_index(
        op.f("ix_activity_timeline_entity_id"), table_name="activity_timeline"
    )
    op.drop_index(
        op.f("ix_activity_timeline_entity_type"), table_name="activity_timeline"
    )
