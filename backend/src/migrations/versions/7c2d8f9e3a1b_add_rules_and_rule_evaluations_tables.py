...
down_revision: str | Sequence[str] | None = "5b9a87680ce9"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "score_impact", sa.Integer(), nullable=False, server_default=sa.text("0")
        ),
        sa.Column(
            "is_blocking", sa.Boolean(), nullable=False, server_default=sa.text("false")
        ),
        sa.Column(
    ...
    op.create_index(
        op.f("ix_rules_organization_id"), "rules", ["organization_id"], unique=False
    )
    op.create_index(
        op.f("ix_rules_created_by_id"), "rules", ["created_by_id"], unique=False
    )
    ...
    op.create_index(
        op.f("ix_rule_evaluations_candidate_id"),
        "rule_evaluations",
        ["candidate_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_rule_evaluations_job_id"), "rule_evaluations", ["job_id"], unique=False
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        op.f("ix_rule_evaluations_candidate_id"), table_name="rule_evaluations"
    )
